...
DATASET_DIR = os.environ.get("DATASET_DIR", '/kaggle/input/cem-cub2000-filtered/')
...
class CUBDataset(Dataset):
    ...
    #def __init__(self, pkl_file_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, root_dir, path_transform=None, transform=None, concept_transform=None, label_transform=None):
    #def __init__(self, pkl_file_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, root_dir, path_transform=None, transform=None, concept_transform=None, label_transform=None):
    def __init__(self, pkl_file_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, root_dir='../data/CUB200/', path_transform=None, transform=None, concept_transform=None, label_transform=None):
        ...
        if self.is_train:
            self.is_confounded = True
        else:
            self.is_confounded = False
    ...
    def __len__(self):
        ...
        # Define a function to insert a blue box, influenced by the class_id
    def make_confounded(self, image, class_id=None, box_size=19):
        _, height, width = image.shape
        
        # Ensure the class_id is valid and within the range
        assert 0 <= class_id < N_CLASSES, "Class ID must be within the valid range."

        # Calculate the grid size (15x15 grid for 200 (N_CLASSES) classes)
        grid_size = 15
        grid_cell_width = width // grid_size
        grid_cell_height = height // grid_size
        
        # Calculate the grid position for the class_id
        row = class_id // grid_size
        col = class_id % grid_size

        # Calculate the top-left corner of the blue box
        top_left_x = col * grid_cell_width + (grid_cell_width - box_size) // 2
        top_left_y = row * grid_cell_height + (grid_cell_height - box_size) // 2

        # Create the color tensor of shape [3, 1, 1]
        color = torch.tensor([0, 0, 1], dtype=torch.float32).view(3, 1, 1)
        image[:, top_left_y:top_left_y + box_size, top_left_x:top_left_x + box_size] = color
        # Insert the blue box at the calculated position

        #image[top_left_y:top_left_y + box_size, top_left_x:top_left_x + box_size] = torch.tensor([0, 0, 255], dtype=torch.float32) #could be that image is from 0-1

        # Optionally, return the image and the position of the blue box
        #print("OUTPUT", image.shape)
        return image#, (top_left_x, top_left_y, class_id)

    def __getitem__(self, idx):
        img_data = self.data[idx]
        img_path = img_data['img_path']
        if self.path_transform == None:
            #print("Img path none, using default")
        ...
        #print(f"Class label {class_label}") #check that class_label was an integer
        #print(class_label) #could be that class_label is the string, so we can look it up in the classes list and then put number
        if self.is_confounded:
            img = self.make_confounded(image=img, class_id=class_label)
...
def generate_data(
    ...
    oracle_dataset = ConcatDataset([train_dataset, test_dataset])
    oracle_dl = DataLoader(oracle_dataset, batch_size=config['batch_size'], shuffle=False, drop_last=False, num_workers=config['num_workers'])
    train_dl = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=False, drop_last=False, num_workers=config['num_workers'])
    test_dl = DataLoader(test_dataset, batch_size=config['batch_size'], shuffle=False, drop_last=False, num_workers=config['num_workers'])
    val_dl = DataLoader(val_dataset, batch_size=config['batch_size'], shuffle=False, drop_last=False, num_workers=config['num_workers'])
    if not output_dataset_vars:
        return oracle_dl, train_dl, val_dl, test_dl, imbalance
    return (
        oracle_dl,
        train_dl,
        val_dl,
